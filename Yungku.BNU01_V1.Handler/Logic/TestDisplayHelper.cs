...
namespace Yungku.BNU01_V1.Handler.Logic
{
    ...
    public static class TestDisplayHelper
    {
        ...
        /// <param name="lowLimit">下限</param>
        /// <param name="upperLimit">上限</param>
        /// <param name="testValue">测试值</param>
        /// <param name="testResult">测试结果</param>
        public static void UpdateLeftStation(string testName, string lowLimit, string upperLimit, string testValue, string testResult)
        {
            try
            {
                leftStationIndex++;
                ...
                if (frmAuto != null)
                {
                    ...
                    {
                        // 只有 Product 最终结果才更新统计数据
                        bool shouldUpdateStats = IsProductFinalResult(testName);

                        // 传递工位索引 0 表示左工位，使用6列格式
                        frmAuto.UpdateStationTest(0, leftStationIndex, testName, lowLimit, upperLimit, testValue, testResult, shouldUpdateStats);
                    }));
                }
            }
            catch (Exception ex)
        ...
        /// <param name="testValue">测试值</param>
        /// <param name="testResult">测试结果</param>
        public static void UpdateRightStation(string testName, string lowLimit, string upperLimit, string testValue, string testResult)
        {
            try
            {
                rightStationIndex++;
                ...
                if (frmAuto != null)
                {
                    MyApp.RunOnUI(new Action(() =>
                    {
                        ...
                        // 传递工位索引 1 表示右工位，使用6列格式
                        frmAuto.UpdateStationTest(1, rightStationIndex, testName, lowLimit, upperLimit, testValue, testResult, shouldUpdateStats);
                    }));
                }
            }
            catch (Exception ex)
        ...
        /// <summary>
        /// 【新增】更新左工位的Value列 - 单独更新数值列
        /// </summary>
        /// <param name="testName">测试名称（用于查找对应行）</param>
        /// <param name="testValue">测试值</param>
        public static void UpdateLeftStationValue(string testName, string testValue)
        {
            try
            {
                FormAuto frmAuto = FormMain.GetFormAuto();
                if (frmAuto != null)
                {
                    ...
                    {
                        // 不更新统计数据，只更新Value列
                        frmAuto.UpdateStationTestValue(0, testName, testValue);
                    }));
                }
            }
            catch (Exception ex)
            {
                MyApp.GetInstance().Logger.WriteError($"更新左工位Value列失败: {ex.Message}");
            }
        }

        /// <summary>
        /// 【新增】更新右工位的Value列 - 单独更新数值列
        /// </summary>
        /// <param name="testName">测试名称（用于查找对应行）</param>
        /// <param name="testValue">测试值</param>
        public static void UpdateRightStationValue(string testName, string testValue)
        {
            try
            {
                FormAuto frmAuto = FormMain.GetFormAuto();
                if (frmAuto != null)
                {
                    ...
                    {
                        // 不更新统计数据，只更新Value列
                        frmAuto.UpdateStationTestValue(1, testName, testValue);
                    }));
                }
            }
            catch (Exception ex)
            {
                MyApp.GetInstance().Logger.WriteError($"更新右工位Value列失败: {ex.Message}");
            }
        }
        ...
        /// <summary>
        /// 判断是否为Product产品最终测试结果（需要更新统计）
        /// </summary>
        /// <param name="testName">测试名称</param>
        /// <returns>如果是Product产品最终结果则返回true</returns>
        private static bool IsProductFinalResult(string testName)
        {
            // Product0、Product1 是产品级最终测试结果，需要更新统计
            // 而像"计算器1"、"计算器2"等只是中间步骤，不需要更新统计
